/**
 * Medcare — Medical History Component (Production)
 *
 * Upgrades:
 * - useToast for error notifications
 * - Retry on load failure
 * - useMemo for filtered list performance
 * - Safe symptom tag parsing (no crash if undefined)
 */
...
const parseTags = (item) => {
  if (Array.isArray(item.selectedTags) && item.selectedTags.length) return item.selectedTags;
  return (item.symptoms || "").split(",").map((s) => s.trim()).filter(Boolean);
};
    ...
    >
      ...
      >
        ...
        <div className="bg-gradient-to-r from-primary to-primary-600 p-5 rounded-t-2xl text-white">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="font-display font-bold text-xl">{item.diagnosis}</h3>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-5 space-y-5">
          ...
          {tags.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {tags.map((s, i) => (
                  <span key={i} className="text-xs bg-blue-50 text-primary border border-blue-100 px-2.5 py-1 rounded-full capitalize">
                    {s}
                  </span>
                ))}
              </div>
          ...
          {item.action && (
            <div className={`rounded-xl p-3.5 border ${sevStyle}`}>
              ...
              <div className="space-y-2">
                {item.alternatives.map((a, i) => (
                  <div key={i} className="flex items-center gap-3 bg-gray-50 rounded-xl px-3 py-2.5">
                    ...
                    <p className="flex-1 text-xs font-semibold text-gray-700">{a.disease}</p>
                    <div className="h-1.5 w-16 bg-gray-200 rounded-full overflow-hidden">
                      <div className="h-full bg-gray-400 rounded-full" style={{ width: `${Math.min(a.confidence, 100)}%` }} />
                    </div>
                    <span className="text-xs text-gray-400 w-9 text-right">{a.confidence}%</span>
                  </div>
                ))}
              </div>
            </div>
...
const MedicalHistory = () => {
  ...
  const load = useCallback(async () => {
    ...
    if (error) toast.error("Could not load medical history.");
    else setHistory(symptoms);
    setLoading(false);
  }, [user?.uid, toast]);
            ...
            {SEVERITIES.map((s) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border font-medium transition-all ${
                  filterSev === s ? "bg-primary text-white border-primary" : "bg-gray-50 text-gray-600 border-gray-200 hover:border-primary/40"
                }`}
              >
                {s}
              </button>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {shown.map((item, i) => (
                  <tr key={item.id || i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3.5 pr-4 text-gray-700 max-w-[140px] truncate text-xs" title={item.symptoms}>
                      {item.symptoms}
                    </td>
                    <td className="py-3.5 pr-4 font-semibold text-gray-900 text-xs">{item.diagnosis}</td>
                    ...
                    <td className="py-3.5 pr-4">
                      {item.severity ? (
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${SEV_STYLE[item.severity] || SEV_STYLE.Unknown}`}>
                          {item.severity}
                        </span>
